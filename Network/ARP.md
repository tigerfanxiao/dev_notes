...
一般情况下ARP 请求是由 PC 机发出的, 交换机收到后, 只是帮助泛洪. 交换机收到 ARP Reply 之后也是通过查询 MAC 地址表来准发. 所以在交换机上并没有 ARP 表. 
...
2. 对端主机收到交换机泛洪的 arp 请求后. 发现ARP 头部中有自己的 IP 地址, 询问的是自己的 mac 地址, 会发送 ARP Reply (单播). 单播的目的地址是 PC 的 mac 地址, 源是自己的 MAC 地址. 交换机收到 ARP Reply 后, 通过查询自己的 MAC 地址表, 进行单播转发.  
...
当路由器收到一个 ARP 请求报文时, 发现其请求的 IP 地址不是自己, 则会进行如下操作
