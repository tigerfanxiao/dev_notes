...
学习(Learning): 接口处于此状态时会侦听业务帧, 但是不转发. 但是收到业务帧后会学习. MAC地址. 这个时间也是15秒
...
BPDU的变化, 只能引起端口角色的变化, 不能引起MAC地址表的变化
当拓扑结构发生变化, 端口被阻塞的设备上, 只剩下根端口, 所以还是接受流量, 并不发出流量, 所以网络中的其他设备, 也无法学习到新的mac地址. mac地址表不变, 但是链路已经变动了. 会造成不同. 此时需要被阻塞的设备发送TCN BPDU. 其他设备收到这类的BPDU会把端口从mac地址表中删除, 并转发TCN 到根桥. 此时其他设备在发送BPDU时, 就会重新发送未知单播帧, 这个帧会泛洪. 最后设备学到了新的mac地址


### 设计
当核心层交换机主备互联, 会和接入层交换机, 形成环路. 一般是阻塞接入层交换机的口, 才会不造成环路. 为什么阻塞接入层交换机, 是因为一般做冗余是接入层到核心层的链路. 如果接入层到核心层的一条链路断了. 接入层交换机会立刻发现, 免去了20秒的阻塞到监听的等待时间. 而如果在另一台核心交换机的一侧接口做阻塞, 则需要多等待20秒的时间
