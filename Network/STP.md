...
此时需要被阻塞的设备R3主动发送TCN BPDU (Topology Change Notification). R2在收到TCN后会沿着根端口的方向转发TCN 到根桥R1. 根桥R1会下发TC到网络中的其他设备, 通知大家拓扑发生了变化. R2在收到R1发出的TC后, 会把R3的MAC地址对应的端口从MAC地址表中删除. 此时因为R3对应的接口被删除, R2向R3发送BPDU时就会重新发送未知单播帧, 这个帧会泛洪. 最后R2会从新的接口学到R3的MAC地址
