...
git init .
git branch -M main # 因为git的默认分支是master, github的默认分支是main
# 添加 .gitignore
git add .
...
# === push to github ===
git remote add origin git@github.com:tigerfanxiao/design_pattern.git

git push -u origin main # 推送到远端 origin仓库, main分支
...
git push -u origin new_branch # 或者main以外的分支
```
