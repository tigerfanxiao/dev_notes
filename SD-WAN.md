...
SD-WAN 可以是敏捷和与硬件解耦的, 因为 SD-WAN 可以编程, 所以可以通过联通其他监控软件对链路和应用的流量做实时的监控和报告生成. 
### QoS
...
在华为的方案中, SD-WAN是部署在服务器上, 在服务器上安装 NCE-SD-WAN 作为控制器, FusionCompute 来建立资源池. 所以 SD-WAN 也是一种 NFV 技术, 就是虚拟出交换机. 作为数据面. 
同时 FusionCompute 也会实时探测链路的负载程度, 完成亚秒级的链路切换.

当需要扩容时, 直接把设备接入控制器, 选择相应的角色, 则会自动完成部署
### 传统的企业WAN 方法
...
### 传统企业 WAN 架构的问题
1. 很难根据应用来选择链路. 比如视频会议这类应用对网络的延迟要求很高. 有些公司通过 SLA-EEM 的方案监听每个应用消耗的资源, 来做策略. 这种配置相当复杂
