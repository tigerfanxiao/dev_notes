...
### 跑测试
```shell
# === run with local virtual env activated ===
python -m pytest

# === run with uv ===
# 解决 error: uv trampoline failed to canonicalize script path 问题
uv run python -m pytest # 当前目录下所有能找到的test_file
uv run python -m pytest test_file_path
```
